    ...
    while((getopt_return = getopt_long(argc, argv, ARG_NAME_ALLOWED_SHORT_ARGS, longopts, &option_index)) != -1) {
        ...
        case ARG_NAME_FILE_SHORT[0]:
            if (optarg != NULL) {
                ...
                log_debug("Filename flag argument received: %s", cmd_args->filename_string);
            }
            else {
                log_error("File flag received a bad argument.");
                return false;
            }
            break;
        ...
        case ARG_NAME_INDEXNAV_SHORT[0]:
            if (optarg != NULL) {
                ...
                log_debug("Index navigation flag argument received: %s", cmd_args->index_navigation_string);
            }
            else {
                log_error("Indexnav flag received a bad argument.");
                return false;
            }
            break;
        ...
        default:
            ...
            log_error("We ran into the default with argument: %d", getopt_return);
            return false;
        }
    }
